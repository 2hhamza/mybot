...
async def show_alerts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    alerts = c.execute('''SELECT alert_type, interval 
                        FROM alerts 
                        WHERE user_id = ?''', (user_id,)).fetchall()
    
    if not alerts:
        await query.edit_message_text(
            "⚠️ لم تقم بإضافة أي تنبيهات بعد\n"
    ...
    keyboard = []
    for alert in alerts:
        btn_text = f"{alert[0]} - كل {alert[1]} دقيقة"
        keyboard.append([InlineKeyboardButton(btn_text, callback_data=f"edit_{alert[0]}")])
...
async def send_alerts(context: ContextTypes.DEFAULT_TYPE):
    now = datetime.now()
    alerts = c.execute('''SELECT user_id, alert_type, interval 
                        FROM alerts 
                        WHERE next_send <= ?''', 
...
if __name__ == '__main__':
    TOKEN = "7543964180:AAHPhEJ8TOcENqsM-FXqkFUJaUhNrBbV8r8"  # استبدل بالتوكن الحقيقي
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            MAIN_MENU: [
                ...
                CallbackQueryHandler(handle_main_menu, pattern='^.*$')  # معالجة أي أحداث أخرى
            ],
